...
import 'package:practice_4/pages/catalog_page.dart'; // Можно удалить этот импорт, если он не используется
import 'package:practice_4/pages/cart_page.dart';
...
class _HomePageState extends State<HomePage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Каталог цветов'),
        actions: [
          IconButton(
        ...
        child: GridView.builder(
          ...
          itemBuilder: (context, index) {
            return ItemNote(
              ...
              isFavourite: favoritesManager.favorites.contains(_flowersList[index]),
              onFavouriteToggle: () => favoritesManager.addToFavorites(_flowersList[index]),
              onAddToCart: () => cartManager.addToCart(_flowersList[index]),
              onRemove: () => _deleteFlower(index),
            );
          },
        ),
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => AddFlowerPage(onAddFlower: _addFlower),
          ),
        ),
